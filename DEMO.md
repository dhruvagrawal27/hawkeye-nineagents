...
- All 5 service health dots green (Postgres / Redis / Neo4j / Kafka / **LLM — confidential-AI · TEE-attested**)
...
- **Investigation memo** — 4-paragraph memo (`Risk Summary` / `What We Observed` /
  `Why It Matters` / `Recommended Next Step`) with audit-trail footer. Generated by
  `openai/gpt-oss-120b` inside an Intel TDX + NVIDIA H200 confidential compute enclave
  (NEAR AI Cloud). Look for the **emerald "Generated in a TEE-attested confidential enclave"** card
  underneath — click *show proof* to reveal the live ed25519 signing key + Intel TDX quote fingerprint.
...
| Narrative panel hangs | Silent provider failover + Jinja fallback should kick in — refresh; if still hung, `docker compose logs backend \| grep narrative` |
