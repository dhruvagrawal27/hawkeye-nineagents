...
## Confidential AI for narrative generation

The investigation memo attached to every alert is generated by `openai/gpt-oss-120b` running inside an **Intel TDX + NVIDIA H200 GPU confidential compute enclave**. Each request to the gateway returns a cryptographic attestation proving the model and prompt executed inside a genuine, unmodified TEE — the cloud operator cannot read the alert payload, the prompt, or the model state.

### What HAWKEYE stores about the TEE state

At backend boot we fetch the gateway's TDX quote once and cache:

- `signing_address` — the ed25519 key the gateway uses to authenticate every inference response
- `intel_quote` — the raw ≈5 KB TDX attestation evidence
- `intel_quote_sha256` — fingerprint reviewers can quote in audit reports

Per-narrative we persist `provider` and `tee_attested` columns on the `narratives` table, so the alert UI can render a verifiable "Generated in a TEE-attested confidential enclave" panel against each memo. The full proof is also exposed at [`/api/attestation`](/api/attestation) for independent verification, and surfaces inside `/api/readyz → llm`.

### Why this matters for RBI

Insider-fraud investigation memos describe specific employees and the SHAP factors that flagged them. Treating this payload as plaintext on shared inference infrastructure is a leakage vector. Processing it inside a hardware-attested TEE means the bank can hand over fraud signals to a managed LLM without trusting the cloud provider's hypervisor, sysadmins, or any future supply-chain compromise of the inference stack. The TEE is the substitute for "self-host the LLM" — same security boundary, none of the GPU CapEx.
